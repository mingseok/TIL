class LinkedList {
    ...
    void delete(int d) {
        ...
        while (n.next != null) { // 마지막 노드까지 찾는다.
            if (n.next.data == d) { // 그 다음 노드의 데이터가 d 인것을 찾는다. (main을 보면 현재 d 값은 1이다. 그래서 바로 if문 조건을 들어 올수 있는 것이다.)
                ...
                n = n.next; // data 2, 3, 4 계속해서 n.next 값이 null인 곳까지 찾는다. 
                            // 찾았다면 종료. 이렇게 하는 이유는 while문을 끝까지 돌리기 위해서도 있지만, 데이터를 지워야 할 중복값이 있을수도 있기 때문이다.
            }
        }
    }
