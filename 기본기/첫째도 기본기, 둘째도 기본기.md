...
‘/save’ 로 한다면 http://localhost:8080/save 이렇게 들어가는 것이다.

### ‘상대 경로’ : “test.txt” → **최상위 /를 거치지 않고도 이동이 가능하다.**

‘/’이 없이 그냥 ‘save’ 되어 있다면 ‘상대 경로’ 인 것이다.

그러면 http://localhost:8080/servlet-mvc/members/new-form 로 시작하여

<form action="save" method="post"> 했다면 

http://localhost:8080/servlet-mvc/members/new-form save 이렇게 끝에만 변하는 것이다.
