...
"member 는 타입이다" 만 생각하고 끝내지 말고, <br/>member는 객체라는 생각과 그 안에 무엇이 있는지 생각해보도록 하자.  
...
1. sessionCookie.getValue() 에서 sessionCookie() 메서드에서 해당 쿠키 객체를 가져온다.

2. 그러면 sessionCookie.getValue() 니깐 .getValue() 를 하면 UUID를 가져 오는 것이다. 
3. 이유는?? 밑에 코드에 보면 두번째 자리가 sessionId 인 때문이다. 
<br/>
(sessionId는 위 코드를 보면 uuid 생성 과정인 것이다.)
<br>
그리하여 왜 getValue() 인가 설명하면 Cookie 클래스에 들어가보면 매개변수 두번째로 자리로 넘어가는 <br> sessionId는 value 자리이기 때문에 uuid를 꺼내는 것이다.
...
4. 여기까지 정리하면 (sessionCookie.getValue()) 부분은 해당 객체안에 저장되어 있는 Value를 가져 오는 것이다. <br/> 밑에 코드가 Cookie 클래스의 내부 생성자이다.
...
5. 그 다음 sessionStore.get(UUID) 로 sessionStore 맵에 UUID를 키로 하여 안에 저장되어 있는 벨류인 객체를 가져오는 것이다.

<br/>

###  한줄 정리

해당 객체를 가져오고, 그 객체안에 있는 uuid를 가져와서 세션스토어에 '키'로 사용하여 

세션스토어 안에 키에 맞는 벨류를 꺼내 오는 것이다.